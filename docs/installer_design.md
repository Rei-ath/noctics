...
2. Collect the generated files under `dist/`:
   ...
   - `installer_manifest.json` with per-slug metadata that now includes the
     archive `size` in bytes alongside `url` and `sha256`.
...
## Telemetry follow-up
The bootstrapper currently ensures configuration directories exist but does not
emit telemetry. When the heartbeat service lands, wire a "install_complete"
event into the bootstrapper (post-install) and a periodic runtime ping so core
distributions and installer flows both report anonymised statistics (counts,
platform/arch, version). Keep payloads lightweight: install timestamp, runtime
version, platform slug, and whether an external instrument was configured.
