...
QK8_0 = 32
BLOCK_Q8_0_BYTES = 2 + QK8_0  # f16 scale + 32 int8
...
def main():
    ...
    if ttype != 8:
        raise SystemExit(f"tensor {name} is type {ttype}, need Q8_0 (type 8)")
    ...
    if cols % QK8_0 != 0:
        raise SystemExit("cols must be multiple of 32 for Q8_0")

    row_blocks = cols // QK8_0
    row_bytes = row_blocks * BLOCK_Q8_0_BYTES

    out_path = Path(args.out)
    with path.open("rb") as f, out_path.open("wb") as out:
        base = data_offset + offset
        for r in range(rows):
            row_off = base + r * (k // QK8_0) * BLOCK_Q8_0_BYTES
            f.seek(row_off)
            for _ in range(row_blocks):
                block = f.read(BLOCK_Q8_0_BYTES)
                if len(block) != BLOCK_Q8_0_BYTES:
