...
detects the host platform, downloads the matching bundle, and installs it. Add
the printed shim directory to `PATH`, then either complete the wizard or set
...
## Distribution Steps
1. Run `./scripts/build_release.sh` on the
...
2. Collect the generated files under `dist/`:
   ...
   - `installer_manifest.json` with per-slug metadata (url, checksums, version, build id).
...
{
  "linux-x86_64": {
    "url": "https://github.com/noctics/noctics/releases/download/v0.1.39/noctics-core-linux-x86_64.tar.gz",
    "sha256": "…",
    "size": 123456789,
    "version": "0.1.39",
    "build": "abc1234"
...
- Whether to ship an installer per platform (MSI/pkg) in addition to the
  bootstrapper.
- Whether to add optional post-install downloads (models, runtimes) later.
