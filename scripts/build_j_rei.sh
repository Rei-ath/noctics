...
SPEC_FILE="$ROOT_DIR/release/noctics_j_rei.spec"
...
if [[ -z "$MODEL_PATH" ]]; then
  ...
  if [[ "${NOCTICS_SKIP_ASSET_PREP:-0}" != "1" ]]; then
    if [[ -f "$MODEL_POINTER_FILE" && -d "$ROOT_DIR/assets/ollama/models/blobs" ]]; then
      echo "[build_j_rei] Reusing existing model assets under assets/ollama/models" >&2
    else
      PREP_SCRIPT="$ROOT_DIR/scripts/prepare_assets.sh"
      if [[ ! -x "$PREP_SCRIPT" ]]; then
        echo "[build_j_rei] Asset prep script missing: $PREP_SCRIPT" >&2
        exit 1
      fi
  ...
  if [[ -z "$MODEL_PATH" && -f "$MODEL_POINTER_FILE" ]]; then
    ...
    echo "[build_j_rei] Unable to determine model path; run prepare_assets first" >&2
    exit 1
  fi
...
if [[ ! -f "$MODEL_PATH" ]]; then
  echo "[build_j_rei] MODEL_PATH does not exist: $MODEL_PATH" >&2
  exit 1
fi

if ! command -v pyinstaller >/dev/null 2>&1; then
  echo "[build_j_rei] pyinstaller not found on PATH" >&2
  exit 1
fi

mkdir -p "$DIST_DIR" "$BUILD_DIR"
rm -rf "$DIST_DIR/noctics-j-rei"
...
echo "[build_j_rei] Personal build available under $DIST_DIR/noctics-j-rei" >&2
