...
func (c *Context) StateSaveFile(path string, tokens []int) error {
	cp := C.CString(path)
	defer C.free(unsafe.Pointer(cp))

	var ctoks []C.llama_token
	var ptr *C.llama_token
	if len(tokens) > 0 {
		ctoks = make([]C.llama_token, len(tokens))
		for i, tok := range tokens {
			ctoks[i] = C.llama_token(tok)
		}
		ptr = (*C.llama_token)(unsafe.Pointer(&ctoks[0]))
	}

	ok := bool(C.llama_state_save_file(c.c, cp, ptr, C.size_t(len(tokens))))
	runtime.KeepAlive(ctoks)
	if !ok {
		return errors.New("failed to save state file")
	}
	return nil
}

func (c *Context) StateLoadFile(path string, maxTokens int) ([]int, error) {
	if maxTokens <= 0 {
		maxTokens = 1
	}
	cp := C.CString(path)
	...
	ctoks := make([]C.llama_token, maxTokens)
	var count C.size_t
	ok := bool(C.llama_state_load_file(
		c.c,
		cp,
		(*C.llama_token)(unsafe.Pointer(&ctoks[0])),
		C.size_t(maxTokens),
		&count,
	))
	if !ok {
