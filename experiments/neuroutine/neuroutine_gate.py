...
def main() -> int:
    ...
    parser.add_argument("--steps", type=int, default=4, help="Tokens to generate")
    parser.add_argument("--no-fast", action="store_true", help="Disable -fast preset")
    parser.add_argument("--weights", default=None, help="Path to controller weights JSON")
    ...
    for prompt in prompts:
        ...
        for step in range(1, args.steps + 1):
            ...
            if not accept:
                start = time.perf_counter()
                chosen = send_prompt(large_proc, current)
                large_time = time.perf_counter() - start
                decision = "fallback-large"
